...
Hard production rule: final detail-page cut images, including sales drafts and concept cuts, must be produced with a generative image model. Do not create final cut images through SVG, HTML screenshots, Canvas rendering, Sharp/ImageMagick composition, deterministic text overlay, Photoshop-style compositing, presentation slides, or manual post-processing. If a generative image model is unavailable or blocked, stop and report the blocker instead of creating fallback images.
...
- Use the generative image model itself to render the Korean text in the final image. Do not use external text overlay, deterministic rendering, screenshot capture, or post-processing as the final-image production path.
...
- If Korean text is missing, broken, unreadable, translated to English, or different from the approved copy, regenerate the image before delivery. Do not patch the text with overlays or manual editing.
